...
const DEFAULT_WRANGLER = path.resolve(here, "..", "wrangler.holotype.toml");
...
interface Args {
  ...
  seedBase?: string;
  chainId?: string;
  /** "" = the default wrangler.holotype.toml; a path = that file; undefined = don't read wrangler. */
  fromWrangler?: string;
  noWrangler?: boolean;
}
...
/**
 * Minimal read of a wrangler config's `[vars]` block into the Env shape loadConfig expects — so the offline
 * CLI rebuilds the SAME brain the deployed Worker runs, instead of the coded fly-brain defaults.
 ...
 *
 * A missing file is a hard error, never a silent fallback to defaults: the whole point is that the hash
 * reflects the deployed sizing, and quietly rebuilding some other brain would anchor a lie on-chain.
 */
function parseWranglerVars(file: string): Partial<Env> {
  if (!existsSync(file)) {
    throw new Error(`wrangler config not found: ${file} (pass --from-wrangler <path>, or --no-wrangler to build from code defaults)`);
  }
...
async function obtainManifest(a: Args): Promise<{ manifest: BrainManifest; source: string }> {
  ...
  // Offline assembly. Unless --no-wrangler, seed the env from the deploy config's [vars] so the rebuild
  // uses the DEPLOYED sizing, then let explicit CLI flags win.
